...
public class SqlOperationUserTable {
    ...
    public void deleteSharedAcessAndDevices(final String sharedHome, final ArrayList<SharingModel> sharedDevices) {
        try {
            AmazonDynamoDBClient dynamoDBClient = new AmazonDynamoDBClient(AWSMobileClient.getInstance().getCredentialsProvider());
            this.dynamoDBMapper = DynamoDBMapper.builder()
                    .dynamoDBClient(dynamoDBClient)
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    UserTableDO user = dynamoDBMapper.load(UserTableDO.class, Constant.IDENTITY_ID);
                    ArrayList<Integer> indexes = new ArrayList<>();
                    for (SharingModel x : sharedDevices) {
                        for(int i=0; i<user.getDevices().size(); i++){
                            String deviceId = user.getDevices().get(i);
                    ...
                    for(int x : indexes) user.getDevices().remove(x);


                    ArrayList<Integer> index = new ArrayList<>();
                    String homeShared = sharedHome.substring(0,sharedHome.length()-7);
                    String masters = sharedDevices.get(0).getMaster();
                    boolean home = false, userId = false, access = false;
                    for (int i=0; i<user.getSharedAccess().size(); i++) {
                        if (user.getSharedAccess().get(i).get("Home").equals(homeShared)) home = true;
                        if (user.getSharedAccess().get(i).get("UserID").equals(masters)) userId = true;
                        if (user.getSharedAccess().get(i).get("Access").equals("accepted")) access = true;

                        if (home && userId && access) {
                            index.add(i);
                        }
                    }
                    for(int x : index) user.getSharedAccess().remove(x);
