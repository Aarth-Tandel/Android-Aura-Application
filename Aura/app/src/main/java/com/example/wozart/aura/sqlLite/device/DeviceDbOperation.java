...
import static com.example.wozart.aura.sqlLite.device.DeviceContract.DeviceEntry.ACCESS;
...
public class DeviceDbOperation {
    ...
    public void getAll(SQLiteDatabase db){
        ...
        while (cursor.moveToNext()) {
            ...
            dummyDevice.setHome(cursor.getString(5));
            dummyDevice.setRoom(cursor.getString(6));
            dummyDevice.setThing(cursor.getString(7));
            access =cursor.getString(8);
            uiud = cursor.getString(9);
    ...
    public void devicesFromAws(SQLiteDatabase db, ArrayList<DevicesTableDO> devices) {
        ...
        for (DevicesTableDO x : devices) {
            ...
            if (!isThingAlreadyPresent) {
                ...
                value.put(DEVICE_NAME, x.getName());
                if (x.getMaster().equals(Constant.IDENTITY_ID)) {
                    value.put(HOME_NAME, x.getHome());
                    value.put(ACCESS, "master");
                }
                else {
                    value.put(HOME_NAME, x.getHome() + "(Guest)");
                    value.put(ACCESS, "guest");
                }
                value.put(UIUD, x.getUIUD());
